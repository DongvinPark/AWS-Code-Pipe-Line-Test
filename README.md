...
## 2026년 개정판 - 추가 정보
- AWS 환경에서 ECR, ECS Blue/Green, CodePipeline 을 이용한 CI/CD 파이프라인을 다시 만들어보면서 가장 헷갈리고 어려운 지점들을 다시 정리했습니다.
- 파이프라인은 Source, Build, Deploy 단계로 이루어져 있습니다.
- 소스와 빌드 단계는 어렵지 않습니다. 문제는 Deploy 단계를 정의하는 것인데, 이때 ALB 와 대상그룹을 정확하게 셋팅해주는 것이 가장 중요합니다.
- 아래의 본문에서 이어지는 2023년 버전의 설명은 스프링 프로젝트를 80 포트에서 돌렸습니다. 따라서 모든 기준 포트가 80으로 맞춰져 있습니다.
- 그러나 80 포트는 루트 권한 없이는 열 수 없으므로 실제 서비스용 서버를 배포할 때는 8080 류의 포트를 써야 합니다.
- 현재의 설명에서는 스프링부트 컨테이너의 포트가 8080이라고 가정하겠습니다.
- 대상 그룹 두 개를 만들어서 ALB 리스너와 매핑할 때는 아래의 안내에 따라서 포트를 반드시 정확하게 매칭하길 바랍니다.
  ...
  - 대상그룹을 만들 때는 VPC가 여러 개일 경우 VPC를 정확하게 선택해야 합니다. 대부분은 default로 하면 됩니다. 왜나면 VPC를 다수를 운영하지 않는 이상 그게 유일한 VPC이기 때문입니다. 그리고 서브넷 IP는 추가하면 안 됩니다.
...
- CodeDeploy에서 배포 애플리케이션과 해당 애플리케이션 내의 배포그룹을 설정하게 될 것입니다.
  - 이때, 배포그룹에서 프로덕션 리스너 포트는 8080 으로, 테스트 리스너포트는 8081 로, 대상 그룹 1 이름은 위에서 만든 'ALB 8080 리스너에 연결된 대상그룹의 이름'으로, 대상 그룹 2는 'ALB 8081 리스너에 연결된 대상그룹의 이름'으로 설정해야 합니다.
  - '원래 개정 종료'에서는 대기 시간을 필요에 따라서 조절하길 바랍니다. 테스트 용일 때는 가장 짧은 5분이 좋겠습니다.
- HTTPS 인증서를 붙일 때는 이렇게 하면 됩니다.
  - 우선 '가비아' 같은 도메인 회사에서 도메인을 하나 구매해 놔야 합니다. 예를 들어 'test.com'이라 하겠습니다. 
  - 그 후, AWS의 Route 53 서비스로 이동해서 'test.com'에 대한 호스팅 영역을 만들어야 합니다. 만들고 나면 'test.com'에 대해서 생성된 NS 레코드 4 개를 확인할 수 있습니다.
  - Route 53에서 만들어진 'test.com'의 호스팅 영역에 있는 NS 레코드 4개를 복사해서 가비아의 네임서버 리스트에 하나씩 복사/붙여넣기 해줍니다. 이때 마지막 마침표는 지워야 합니다. 그후 가비아에서 최종적으로 네임서버 변경을 실행합니다. 도메인 소유자의 휴대폰 또는 이메일 인증이 요구될 수 있습니다.
  - 그 후, AWS로 이동해서 Certificate Manager 서비스로 이동한 후 ACM 인증서를 '*.test.com' 이런 식으로 와일드 카드를 써서 요청합니다. 요청 옵션을 선택하는 화면에서 인증서 내보내기는 '비활성화'로 선택해서 신청합니다. 그래야 인증서 갱신 등의 귀찮은 작업을 AWS에게 모두 맡길 수 있습니다. 검증 방법은 'DNS 검증'을 선택하고, 알고리즘은 RSA 2048을 선택해서 최종 요청합니다.
  - 요청 완료 후 '*.test.com' 인증서의 상세 화면으로 들어갑니다. 그러면 'Route 53에서 레코드 생성'이라는 버튼이 보일텐데 이것을 클릭합니다. 그러면 AWS가 알아서 Route 53에 인증서 검증용 CNAME 레코드를 추가하고, 몇 분 안에 인증서 상태가 발급됨(Issued)으로 바뀔 것입니다.
  - ALB에 443 포트 HPPTS 리스너를 추가합니다. 발급 받은 인증서를 선택합니다.
  - Route 53에 들어가서 레코드를 추가합니다. 현재 '*.test.com'에 대해서 ACM이 발급된 상태이기 때문에 실제로 사용할 https 서브 도메인을 Route 53에 정의해서 '레코드 추가'를 해야 합니다.
  - Route 53 > 호스팅 영역 > '*.test.com' 클릭 > [레코드 생성] 클릭.
  - 레코드 생성 화면에서 '레코드 이름'은 qa 라 하겠습니다. 레코드 타입은 'A(별칭 Alias)'으로 하고, 별칭 대상은 ALB 로 지정합니다.
  - 이제부터 'https://qa.test.com' 도메인으로 들어오는 모든 https 요청은 우리가 만든 ALB로 라우팅 됩니다.
  - Code Pipeline에서 'Deploy' 단계로 들어가서 '배포 그룹'을 수정합니다.
    - 프로덕션 포트는 443 으로 하고, 테스트 포트는 8080 으로 합니다.
    - 대상 그룹 1은 원래 8080 리스너에 붙어 있던 대상 그룹으로 설정하고,
    - 대상 그룹 2는 원래 80801 리스터에 붙어 있던 대상 그룹으로 설정해서 배포그룹을 수정합니다.
  - 커밋을 하나 날려서 파이프라인을 처음부터 재가동 시킨 후 모든 단계들이 완료될 때까지 기다립니다.
  - 그러면 'https://qa.test.com/swagger-ui/index.html' 이 뜨는 것을 확인할 수 있습니다.
  - 남은 ALB에서 남은 8081 리스너는 ALB에서 제거하고, 8080 리스너는 http 이므로 인바운드 규칙을 '내 IP' 이런 걸로 수정해 놓습니다.
  - 이제부터는 파이프라인이 돌 때마다 443 포트와 8080 포트가 둘다 대상 그룹 1 또는 2를 가리키면서 CI/CD 파이프라인에 의한 배포가 진행됩니다.
